...
export function OrderDialog() {
	const { data: orders, mutate: mutateOrders } = useSWR("customer_orders", () =>
		getSuplierOrders(),
	);
	...
	>([{ article: "", name: "", supplier_id: "", price: 1, quantity: 1 }]);
	...
	return (
		...
		>
			<Dialog.Trigger asChild>
				<button className="bg-fuchsia-400 hover:bg-fuchsia-500 text-white px-3 h-10 rounded-lg shadow-md font-semibold transition-all">
					Замовлення
				</button>
			</Dialog.Trigger>
			...
			<Dialog.Portal>
				...
				<Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[90vw] h-[90vh] flex flex-col bg-white rounded-xl shadow-2xl focus:outline-none overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
														...
														{order?.products.map((p, idx) => (
															...
															>
																<div className="flex gap-4">
																	<span className="text-gray-400 text-xs mt-0.5 block w-48">
																		Артикул: {p.article}
																	</span>
																	<span>{p.name}</span>
								...
								<div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
									...
									{customer.restricted === true && (
										<div className="mt-3 p-3 bg-red-100 text-red-800 text-sm font-medium rounded-md border border-red-200 flex items-center">
											⚠️ Цей користувач: цей користувач обмежений у створенні
											замовлень без застави
										</div>
									)}
								</div>
