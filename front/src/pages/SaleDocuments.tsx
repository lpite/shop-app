...
import { Table, TableBody, TableHead, TableRow, TableCell, Button } from "@suid/material"
...
export default function SaleDocuments() {
	...
	onMount(async () => {
		const docs = await fetch(`${API_URL}/get/sale-documents/`).then(res => res.json()).catch(err => [])
		setDocuments(docs)
	})
	...
	return (
		<main>
			<h1>Продаж</h1>
			<Button onClick={()=>openSaleDocument("new")} variant="contained">Новий</Button>
			<Table size="small">
				<TableHead>
					<TableRow>
				...
				<TableBody>
					<For each={documents()}>
						{(doc) => (
							<TableRow onDblClick={() => openSaleDocument(doc.document_id)} sx={{userSelect:"none"}}>
