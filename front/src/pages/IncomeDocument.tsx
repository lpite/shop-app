...
import { Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography } from "@suid/material";
...
export default function IncomeDocument() {
	const [incomeDocument, setIncomeDocument] = createSignal<IncomeDocument>({ time: 0, document_id: 0, products: [], isPosted: false })
	...
	onMount(async () => {
		if (params.documentId !== "new") {
			const doc = await fetch(`${API_URL}/get/income-document/?document_id=${params.documentId}`).then(res => res.json()).catch(err => {})
		...
		if (selectedProducts.length) {
			...
			setIncomeDocument({ ...incomeDocument(), products: [...incomeDocument().products, ...selectedProducts] })
		}
	})
	async function saveDocument() {
		...
		await fetch(`${API_URL}/${params.documentId === "new" ? "create" : "update"}/income-document/?document_id=${incomeDocument().document_id}`, {
	...
	function deleteProduct(index: number) {
		const newProducts = incomeDocument().products.filter((prod, i) => i !== index);
