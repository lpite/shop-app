...
*/
		...
		const product: any = await db.get(`SELECT name,product.product_id,price FROM product LEFT JOIN price ON price.product_id=product.product_id WHERE product.product_id = ?`, [query.product_id])
...
app.get("/get/products/", async (req, res) => {
	try {
		...
		const products = await db.all(`SELECT product.product_id,price.price,name
									FROM product 
...
app.post("/create/price/", async (req, res) => {
	try {
		...
		await db.run(`INSERT INTO price(product_id,price) VALUES(?,?);`, query.for, body.price)
...
app.get("/get/sale-document/", async (req, res) => {
	try {
		...
		const saleDocument = await db.get(`SELECT * FROM sale_document WHERE document_id = ?`, [query.document_id])
		const saleDocumentProducts = await db.all(`SELECT 
							product.product_id,
							product.name,
						...
						WHERE document_product.document_id = ?;`, [query.document_id])
...
app.get("/get/sale-documents/", async (_, res) => {
	try {
		...
		const saleDocuments = await db.all(`SELECT * FROM sale_document ORDER BY time DESC LIMIT 100;`);
...
app.post("/create/sale-document/", async (req, res) => {
	try {
		...
		const quantities = await 
			countProductQuantityExp({ products: body.products });

		for (let i = 0; i < body.products.length; i++) {
			const product = body.products[i] as Product;
			productsQuantity[product.product_id] = (productsQuantity[product.product_id] || 0) + product.quantity
		}
		...
		const result = await db.run(`INSERT INTO sale_document (time,isPosted) VALUES(${getTimeInSeconds()},?)`, body.isPosted)

		for (let i = 0; i < body.products.length; i++) {
			const { product_id, quantity, price } = body.products[i];
			await db.run(`INSERT INTO sale_document_product (document_id,product_id,quantity,price) 
							VALUES(?,?,?,?)
		...
		const saleDocument = await db.get("SELECT * FROM sale_document WHERE document_id = ?", [result.lastID])
		res.send(saleDocument)	
...
app.patch("/update/sale-document/", async (req, res) => {
	...
	*/
	try {
		...
		let error = false;
		await db.exec("BEGIN TRANSACTION;")
		await db.run(`UPDATE sale_document SET isPosted = ? WHERE document_id = ?`, body.isPosted, query.document_id);
		await db.run(`DELETE FROM sale_document_product WHERE document_id = ?`, [query.document_id]);
		for (let i = 0; i < body.products.length; i++) {
			const { product_id, quantity, price } = body.products[i];
			await db.run(`INSERT INTO sale_document_product (document_id,product_id,quantity,price) 
							VALUES(?,?,?,?)
				`, [query.document_id, product_id, quantity, price]).catch((err) => {
...
app.get("/get/income-document/", async (req, res) => {
	try {
		...
		const incomeDocument: IncomeDocument | undefined = 
			await db.get(`SELECT * FROM income_document WHERE income_document.document_id = ?`, 
				[query.document_id])
...
app.get("/get/income-documents/", async (_, res) => {
	try {
		...
		const incomeDocuments: IncomeDocument[] = await db.all(`SELECT isPosted,time,document_id FROM income_document ORDER BY time DESC LIMIT 50;`);
...
app.post("/create/income-document/", async (req, res) => {
	try {
		...
		const result = await db.run(`INSERT INTO income_document (time,isPosted) VALUES(${getTimeInSeconds()},?)`, body.isPosted)
		let error = false
		for (let i = 0; i < body.products.length; i++) {
		...
		const incomeDocument = await db.get("SELECT * FROM income_document WHERE document_id = ?", [result.lastID])
...
app.patch("/update/income-document/", async (req, res) => {
	try {
		...
		await db.run(`UPDATE income_document SET isPosted = ? WHERE document_id = ?`,
			[body.isPosted, query.document_id])

		await db.run(`DELETE FROM income_document_product WHERE document_id = ?`, [query.document_id]);

		for (let i = 0; i < body.products.length; i++) {
			const product = body.products[i]
			await db.run(`INSERT INTO income_document_product(document_id,product_id,quantity,price)
					VALUES(?,?,?,?)
				`, [query.document_id, product.product_id, product.quantity, product.price]).catch((errorMsg) => {
				console.error(errorMsg)
				error = true
			})
		}
